...
const insertMovieGenres = (
  ...
  genreRows: GenreRow[]
   ...
   genres.map(g => `(${movieId}, ${(genreRows.filter(row => row.genre == g))[0].id})`).join(",") ;   
};
...
const insertMovieActors = (
  ...
  actors.map(a => `(${movieId}, ${(actorRows.filter(row => row.full_name == a))[0].id})`).join(",");
};
...
const insertMovieDirectors = (
  ...
  directors.map(d => `(${movieId}, ${(directorRows.filter(row => row.full_name == d))[0].id})`).join(",");
};
...
const insertMovieKeywords = (
  ...
  keywords.map(k => `(${movieId}, ${(keywordRows.filter(row => row.keyword == k))[0].id})`).join(",");
};
...
const insertMovieProductionCompanies = (
  ...
  productionCompanies.map(p => `(${movieId}, ${(productionCompanyRows.filter(row => row.company_name == p))[0].id})`).join(",");
};
  ...
  it(
    ...
    async done => {
      ...
      const keywordRows = (await db.selectMultipleRows(
        `select id, keyword from keywords`)) as KeywordRow[];
      const moviesByImdbId = _.groupBy(await CsvLoader.movies(), "imdbId");
